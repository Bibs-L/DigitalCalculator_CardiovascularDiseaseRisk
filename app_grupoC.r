...
server <- function(input, output, session) {
  ...
  output$download_relatorio <- downloadHandler(
    ...
    content = function(file) {
      ...
      if (!is.null(risco_guardado)) {
        ...
        if (risco_guardado[2]<=200 & risco_guardado[4]<=130 & risco_guardado[5]==0) {
          grid.text(paste("Continue a manter os seus bons hábitos!"), x = 0, y = 0.32, just = "left", gp = gpar(fontsize = 11))
        }
  ...
  observeEvent(input$processar, {
    ...
    colnames(dados) = c("nome_utente","sexo","idade", "col", "HDL", "PAS", "tratamento", "fumador", "diabetico", "risco")
    ...
    colnames(dados_tabela) = c("Nome de Utente", "Sexo", "Idade", "Colesterol Total", "HDL", "Pressão Arterial Sistólica", "Em Tratamento", "Fumador", "Diabético", "Risco")
